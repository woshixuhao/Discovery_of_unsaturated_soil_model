...
if __name__ == '__main__':
    ...
    mode='Valid' #['Train','Valid_discovered']


    t_for_prediction=np.arange(0,1,0.01)
    psi_for_prediction=10**(np.flip(np.arange(0,5,0.1),axis=0))
    ...
    if mode=='Valid':
        ...
        for n in range(len(theta)):
            ...
            upper_bound = pred_theta_from_psi[-1]  # upper_bound is where h=1
            lower_bound = pred_theta_from_psi[0]  # low_bound is where h=1e5
            print("non negative theta:", lower_bound,upper_bound)
            expr_Kr = calculate_Kr(sim_expr[n], x, lower_bound,upper_bound)
    ...
    if mode=='Train':
        # ==============Regression====================
        GA.evolution(save_dir='discovered_results')